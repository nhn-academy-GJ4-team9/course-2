...
//    }  // end main()
    ...
    public static void main(String[] args) {
        InputStream file = Ex9_2.class.getClassLoader().getResourceAsStream("test.txt");
        readFile(file);

    }

    public static TreeNode readFile(InputStream file){
        String item;
        try(BufferedReader br = new BufferedReader(new InputStreamReader(file))) {
            while ((item=br.readLine())!=null){
                item = br.readLine().trim().toLowerCase();
                if(item.length()==0)
                    break;
                if(treeContains(root,item)){
                    System.out.println("\nThat item is already in the tree.");
